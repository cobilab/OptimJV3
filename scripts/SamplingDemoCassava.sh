...
for size in ${sampleSizes[@]}; do
    dsx="$(awk '/cassava'$size'[[:space:]]/ { print $1 }' "$ds_sizesBase2")"
    gaFolder="../$dsx/sampling100gens"
    gaFolder200="../$dsx/sampling200gens"
    [ -d $gaFolder200 ] && mv $gaFolder200 ${gaFolder200}_bkp
    cp -r $gaFolder $gaFolder200
done

# run GAs
# (lowering number of selected cmds, and crossover rate of commands and models between generation 101 and 200)
for sampleSize in "${sampleSizes[@]}"; do
    bash -x ./GA.sh -s cassava$sampleSize -ga sampling200gens -hyi -hhp 0.5 -sl "rws" -cc "rmga" -c "u" --moga -wBPS 0.5 -fg 101 -lg 200 1> out$sampleSize 2> err$sampleSize &
    wait
done
